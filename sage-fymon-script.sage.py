# This file was *autogenerated* from the file /Users/nnepal2/Documents/GitHub/jupynote/sage-fymon-script.sage
...
maxChains = [[list(chain) for chain in p][_sage_const_1 :] for p in frozenChains if len(p) == B.rank() + _sage_const_1 ]
    ...
    for i in range(_sage_const_1 , rank):
        for j in range(rank):
            ...
            if rank - (i+_sage_const_1 ) > _sage_const_1 :
                y = generate_weights(rank - (i + _sage_const_1 ))  # Recursion to get the complete list of weights...
                for x in y:
...
def generate_conjugacyClasses(n):
    def partition(n, m=None):
        ...
        if n == _sage_const_0 :
            return [[]]
        if n < _sage_const_0  or m == _sage_const_0 :
            return []
        partitions = partition(n, m - _sage_const_1 )
    ...
    for partition in subdivided_partitions:
        ...
        for x in partition:
            for i, element in enumerate(x):
                conjugacy_class[element] = x[(i + _sage_const_1 ) % len(x)]
...
def apply_map(monomial, mapping):
    ...
    for term in monomial:
        new_term = []
        for edge in term[_sage_const_0 ]:
...
def monomial_equality(monomial, mapped_monomial):
    ...
    sorted_monomial = [(sorted([tuple(sorted(segment)) for segment in m[_sage_const_0 ]]), m[_sage_const_1 ]) for m in monomial]
    sorted_mapped_monomial = [(sorted([tuple(sorted(segment)) for segment in m[_sage_const_0 ]]), m[_sage_const_1 ]) for m in mapped_monomial]
...
def character(temp_fyMonomials, conjugacy_Class):
    ...
    for monomial in temp_fyMonomials:
        ...
        if monomial_equality(monomial, mapped_monomial): i += _sage_const_1 
    return i


characters = [character(mutable_fyMonomials[_sage_const_1 ], mapping) for mapping in conjugacy_Classes]
